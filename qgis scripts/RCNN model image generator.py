...
dx_range = (-extent_offset * 0.5, extent_offset * 0.5)  # Range for random dx, dy offsets
...
def process_feature(feature, is_positive):
    ...
    if is_positive:
        ...
        dy = random.uniform(*dx_range)
        center = QPointF(center.x() + dx, center.y() + dy)
        ...
        if is_positive:
            for other_feature in positive_layer.getFeatures():
                if extent.contains(other_feature.geometry().asPoint()):
                    ...
                    rel_y = (extent.yMaximum() - other_feature.geometry().asPoint().y()) / extent.height() * image_size
